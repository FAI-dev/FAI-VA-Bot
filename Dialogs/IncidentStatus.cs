...
namespace Microsoft.BotBuilderSamples.Dialogs
{
    public class IncDialog : CancelAndHelpDialog
    {
        ...
        private async Task<DialogTurnResult> Commment_ascalateStepConfirmAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            incidentDetails.escalate_option = (string)stepContext.Context.Activity.Text.ToLower();



            if (incidentDetails.escalate_option.Equals("comment"))
            {
                return await stepContext.PromptAsync(nameof(TextPrompt), new PromptOptions { Prompt = MessageFactory.Text("Please enter your comment: ") }, cancellationToken);

              //  return await stepContext.NextAsync(stepContext, cancellationToken);
            }
            else if (incidentDetails.escalate_option.Equals("escalate"))
            {
               return await stepContext.PromptAsync(nameof(TextPrompt), new PromptOptions { Prompt = MessageFactory.Text("You can escalate this issue to: megha.bhamare@fujitsu.com. Kindly post your comments for escalation.") }, cancellationToken);
               // return await stepContext.NextAsync(stepContext, cancellationToken);

            }
            else if (incidentDetails.escalate_option.Equals("satisfied"))
            {

              return  await stepContext.PromptAsync(nameof(TextPrompt), new PromptOptions { Prompt = MessageFactory.Text("Please share your feedback ") }, cancellationToken);
              //  return await stepContext.NextAsync(stepContext, cancellationToken);
            }else
            {
        ...
        {
            var incidentDetails = (IncidentDetails)stepContext.Options;
            if (((IncidentDetails)stepContext.Options).Coment_for_Team == null)
            {
                incidentDetails.Coment_for_Team = (string)stepContext.Result;
            }
            ...
            if ((incidentDetails.Incident_No != null))
            {
...

                {
                    ...
                    incident_Api_Result = nOWLoggerforinc.GetIncident(incidentDetails.Incident_No.ToUpper());
                    string incident_Api_Result1 = nOWLoggerforinc.CommentONIncident(incidentDetails.Incident_No.ToUpper(), incidentDetails.Coment_for_Team);

                    CoreBot.models.apiresult apiresult = nOWLoggerforinc.GetIncidentDetails(incidentDetails.Incident_No.ToUpper());
                    ...
                    if (incident_Api_Result != null && apiresult != null && incident_Api_Result1 !=null)
                    {
                        if (incident_Api_Result.Result.Count != 0 && apiresult.result.Count != 0)
                        {
                            ...
                            reply.Attachments.Add(Cards.GetHeroCardforStatusUpdate(apiresult.result[0].number, incident_Api_Result.Result[0].value, incidentDetails.Coment_for_Team).ToAttachment());
                        ...
                        else
                        {
                            ...
                            {
                                await stepContext.Context.SendActivityAsync(
                          MessageFactory.Text("Incident is updated to team"), cancellationToken);
